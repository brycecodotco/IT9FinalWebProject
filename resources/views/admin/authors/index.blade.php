<x-app-layout>
    <div class="py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <h2 class="font-bold text-3xl text-slate-800 tracking-tight">Manage Authors</h2>
                <p class="text-sm text-slate-500 mt-2">Add new authors to the database or remove unused ones.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-1">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 sticky top-28">
                        <h3 class="font-bold text-lg text-slate-800 mb-4">Add New Author</h3>
                        <form action="{{ route('admin.authors.store') }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label class="block text-xs font-bold text-slate-700 uppercase mb-2">Author Name</label>
                                <input type="text" name="author_name" required class="w-full border-slate-300 rounded-xl shadow-sm px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 rounded-xl shadow-sm transition">Add Author</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="md:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase">Author Name</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($authors as $author)
                                <tr class="hover:bg-slate-50">
                                    <td class="px-6 py-4 text-sm font-bold text-slate-900">{{ $author->author_name }}</td>
                                    <td class="px-6 py-4 text-sm text-center text-slate-600">
                                        <span class="bg-slate-100 text-slate-800 px-2.5 py-1 rounded-full text-xs font-bold">{{ $author->books_count }}</span>
                                    </td>
                            ...
                            @empty
                                <tr><td colspan="3" class="px-6 py-8 text-center text-slate-500">No authors found.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
